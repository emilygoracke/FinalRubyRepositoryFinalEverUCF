...
public class RubyController : MonoBehaviour
{
    ...
    public AudioClip BonesClip;

    public AudioClip CatClip;
    
    public AudioClip DogClip;

    public AudioClip DonutClip;
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.X))
        {
            RaycastHit2D hit = Physics2D.Raycast(rigidbody2d.position + Vector2.up * 0.2f, lookDirection, 1.5f, LayerMask.GetMask("CPC"));
            if (hit.collider != null)
            {
                    CatScript character = hit.collider.GetComponent<CatScript>();
                    if (character != null)
                    {
                        character.DisplayDialog();
                        PlaySound(CatClip);
        ...
        if (Input.GetKeyDown(KeyCode.X))
        {
            RaycastHit2D hit = Physics2D.Raycast(rigidbody2d.position + Vector2.up * 0.2f, lookDirection, 1.5f, LayerMask.GetMask("DPC"));
            if (hit.collider != null)
            {
                    DogScript character = hit.collider.GetComponent<DogScript>();
                    if (character != null)
                    {
                        character.DisplayDialog();
                        PlaySound(DogClip);
    ...
    void SetBonesText()
    {
        BonesText.text = "Bones: " + Bones.ToString();
    }
